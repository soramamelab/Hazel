...
1ハゼを検知すると、画面上部のボタンの右に **DTR** が表示されます。

DTR (%) = 1ハゼ以降の時間 ÷ 投入からの焙煎時間 × 100

- 焙煎中は**その時点まで**で計算するため、時間が進むにつれて増えていきます（オレンジ色）
- **焙煎終了**を押すと、上げまでの**確定値**になります（緑色）
- カッコ内は1ハゼ以降の経過時間（Development Time）です
- 1ハゼを「いいえ（取消）」で取り消した場合は表示されません
- 焙煎終了時の保存ポップアップと、CSV Viewer の情報欄にも表示されます
...
詳細設定の「排出・焙煎終了の自動検出」を ON にすると、豆をドラムから出したときのΔH₂Oの落ち込みから排出を判定し、焙煎を自動で終了します。**初期設定はオフ**です。

判定の条件は次の2つです。

1. ΔH₂Oが、**30秒前までの最大値から設定した割合（初期値 20%）以上**下がる
2. そのまま**15秒間、戻らない**

排気操作やセンサーのノイズでも一時的に落ちることがありますが、そちらはすぐ戻るため停止しません。逆に、豆を出した後はΔH₂Oが戻らないので確実に判定できます。

- **変化速度（g/m³/s）ではなく落ち込みの割合で見ています。** ΔH₂Oの水準は焙煎機やフード形状で数倍違い（実測 10〜48 g/m³）、同じ排出でも変化速度は -0.25〜-0.55 と幅がありますが、落ち込みの割合はどの機種でもよく揃っていたためです
- **焙煎時間は、検知した瞬間ではなく落ち始め**（実際に豆を出した時刻）で記録します。検知は判定に必要な落ち込みが出るまで4〜8秒遅れるためです
- 自動終了すると、ボタンで終えたときと同じく保存ポップアップが開き、グラフとCSVに「上げ」が記録されます。状態表示は「焙煎終了 — 焙煎時間 mm:ss（自動検出）」になります
- **投入から1分間は判定しません。** 投入の瞬間はΔH₂Oが大きく振れるため、そのまま判定すると「投入した直後に排出」と誤って終わってしまいます（過去の焙煎12本で確かめたところ、この種の誤検出はすべて投入から36秒以内に起きていました）
...
| 項目 | デフォルト | 説明 |
|------|-----------|------|
| **自動検出 ON/OFF** | OFF | ΔH₂Oの落ち込みから排出を判定し、焙煎を自動終了する（[3. 焙煎の基本操作](#排出焙煎終了の自動検出)） |
...
- **情報欄**：データ数・経過時間・1ハゼ時刻に加えて、**DTR** を表示します（`first_crack_flag` を持つ CSV の場合）
